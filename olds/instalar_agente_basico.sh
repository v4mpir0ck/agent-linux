...
# Copiar la librería libpython3.10.so.1.0 a /usr/local/lib si no existe
LIBPY_SRC=$(find /usr/lib /lib -name 'libpython3.10.so.1.0' 2>/dev/null | head -n1)
LIBPY_DEST="/usr/local/lib/libpython3.10.so.1.0"
if [ -n "$LIBPY_SRC" ]; then
	if [ ! -f "$LIBPY_DEST" ]; then
		echo "[INFO] Copiando libpython3.10.so.1.0 a /usr/local/lib..."
		$SUDO cp "$LIBPY_SRC" "$LIBPY_DEST"
		$SUDO ldconfig
	fi
else
	echo "[WARN] No se encontró libpython3.10.so.1.0 en el sistema. El binario puede fallar."
fi

echo "[INFO] Probando binario instalado..."
# Exporta LD_LIBRARY_PATH para asegurar que el binario encuentre la librería
...
if echo "$BIN_TEST_OUTPUT" | grep -qE 'GLIBC|libpython|Failed to load Python shared library|No module named'; then
		...
		pip install pyinstaller
			LIBPY=$(find /usr/lib /lib -name 'libpython3.10.so.1.0' | head -n1)
			if [ -z "$LIBPY" ]; then
				echo "[ERROR] No se encontró la librería libpython3.10.so.1.0 en el sistema."
				exit 1
			fi
			pyinstaller /tmp/agente_src/__main__.py --name agente --add-binary "$LIBPY:."
			if [ -d dist/agente ]; then
				...
				echo "[OK] Binario agente compilado localmente en modo onedir con libpython incluida. Ejecuta 'agente' desde cualquier lugar."
			else
				echo "[ERROR] Falló la compilación local en modo onedir con libpython incluida. Revisa dependencias y fuentes."
				exit 1
			fi
fi
fi
